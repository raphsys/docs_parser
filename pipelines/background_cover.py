...
BACKGROUND_ONLY_STATES = {"background_only", "background_visual"}
...
def _has_any(raw: str, markers: set[str]) -> bool:
    # Signatures are whitespace-separated fields.  Substring matching made
    # `non_text_unit` match the marker `text` and promoted containers to text.
    return bool(set(str(raw or "").split()) & markers)
...
def _collect_boxes_from_regions(input_data: dict) -> list[list[float]]:
    ...
    for r in input_data.get("regions") or []:
        if not isinstance(r, dict):
            continue
        region_type = str(r.get("region_type") or "").lower()
...
def collect_background_purity_boxes(input_data: dict) -> list[list[float]]:
    ...
    page_w = float(geom.get("width") or geom.get("width_pt") or 0.0)
    page_h = float(geom.get("height") or geom.get("height_pt") or 0.0)
    page_area = page_w * page_h
    if page_area > 0:
        # A text/formula/code erase box can never legitimately own almost the
        # whole page.  Such a box is a contaminated container ownership claim.
        boxes = [b for b in boxes if _area(b) / page_area < 0.85]
    return boxes
...
def _local_fill_color(img, rect: list[int], fallback: tuple[int, int, int]) -> tuple[int, int, int]:
    ...
    px = img.load()
    pts = []
    # Sample both the text box and its immediate neighbourhood.  The modal
    # quantized colour is normally the substrate; glyph colours are a minority.
    # This works for white paper, beige callouts and dark table headers.
    step = max(1, min(3, (y1 - y0) // 4 or 1))
    for y in range(oy0, oy1, step):
        for x in range(ox0, ox1, step):
            pts.append(px[x, y])
    if len(pts) < 8:
        return fallback
    buckets = Counter((r // 16, g // 16, b // 16) for r, g, b in pts)
    winner, _ = buckets.most_common(1)[0]
    cluster = [p for p in pts if (p[0] // 16, p[1] // 16, p[2] // 16) == winner]
    return tuple(int(median([p[i] for p in cluster])) for i in range(3))
